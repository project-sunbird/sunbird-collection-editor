org.ekstep.services.collectionService = new (Class.extend({
	...
	getContextMenu: function (node) {
		...
		contextMenu = [
			...
			{ title: 'Delete <kbd>[' + del + ']</kbd>', cmd: 'remove', uiIcon: 'ui-icon-trash', disabled: !!node.data.root }
		]
		if ((node.data.metadata.visibility === 'Default' && node.data.root) || (node.data.metadata.visibility === 'Parent') || _.isUndefined(node.data.metadata.visibility)) {
			var menu = [{ title: '----' },
				{ title: 'New sibling <kbd>[' + ctrl + '+' + alt + '+Shift+N]</kbd>', cmd: 'addSibling', uiIcon: 'ui-icon-plus', disabled: !!node.data.root },
	...
	filterResource: function (err, res) {
		if (res) {
			...
			console.error('Fails to filter', err)
		}
	},
	addTree: function (tree) {
		...
		ecEditor.jQuery('#collection-tree').fancytree({
			...
			var tree = ecEditor.jQuery(this).fancytree('getTree')
			...
			if (!keyword.isPresent) {
                ...
                org.ekstep.services.metaService.suggestVocabulary(requestData, function(err, resp) {
					if (resp) {
						...
						reject(new Error('Fails to suggest a vocabulary..', err))
					}
				})
			} else {
